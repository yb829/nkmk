...
%>
<style>
section{width:90%; margin:0 auto 50px auto;}
...
<section>
	<div class="prod_list_wrap">
		...
		<h4 align= "left">01 장바구니 > 02 주문서 작성 결제> 03 주문완료 </h4>
		<table  id="tbl-tap">						
			<tr>
		...
		<h4>일반구매상품</h4>
			<table id="tbl-cart">
				...
				<tr>
					...
					<td>
						<form method="GET" action="<%= request.getContextPath() %>/cart/delete">
							<input type="hidden" name="cartNo" value="<%= cartOn.getCartNo() %>">
					...
					<td>
						<form method="GET" action="<%=request.getContextPath()%>/cart/update">
							<input type="number" name="cartProdCount" class="cartProdCount" value="<%=cartOn.getCartCount()%>">
							<input type="hidden" name="cartNo" value="<%= cartOn.getCartNo() %>">
							<input type="submit" name="modifyBtn" class="modifyBtn" value="수정">
						</form>
					</td>
			...
			<div id="cart-container">
				<div id="cart-sidebar">
					<h5 class="bigtext right-align sumcount" id="sum_p_num"> 
						<%= cartList.size() %>개의 상품금액 - 
						<% 
							int priceSum = 0;
							for(Cart cartOn : cartList){
								priceSum += (cartOn.getCartCount() * cartOn.getPriceSum());
							} 
						%>
						<%= priceSum %>원
					</h5>
					...
					<hr>
					<h5 class="bigtext right-align box blue summoney" id="sum_p_price">
						결제예상금액 : 
						<% if(priceSum >= 30000){ %> 
						<%= priceSum %>원
						<% } else { %>
